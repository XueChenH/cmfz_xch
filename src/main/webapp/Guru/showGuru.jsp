...
<!doctype html>
    ...
    <script>
        $(function () {
            ...
            //通过js创建datagrid
            $("#gurudg").datagrid({
                url:'${pageContext.request.contextPath}/Guru/showByPage',//用来请求远程数据
                ...
                columns:[[
                    ...
                    {title:'上师名字',field:'name',width:120,},
                    {title:'上师头像',field:'headPic',width:120,},
                    {title:'上师状态',field:'status',width:120,sortable:true},
                    {title:'options',field:'options',width:200,
                        formatter:function(value,row,index){
                            return "<a href='javascript:;' class='options' onclick=\"delRow('"+row.id+"')\" data-options=\"iconCls:'icon-remove',plain:true\">删除</a>&nbsp;&nbsp;" +
                                "<a href='javascript:;' class='options' onclick=\"openUpdateGuruDialog('"+row.id+"')\" data-options=\"iconCls:'icon-edit',plain:true\">更新</a>";
                        }
                    }
                ]],
...
/*        //删除
        function delRow(id) {
            $.post("${pageContext.request.contextPath}/Guru/removeAll",{"id":id},function (result) {
        ...
        function delRow(id){
            $.messager.confirm("确认对话框","确定删除么？？",function(b){
                if(b){
                    ...
                    $.post("${pageContext.request.contextPath}/Guru/removeAll",{"id":id},function (result) {//响应成功之后回调
                        //刷新datagrid数据
        ...
        function openSaveGuruDialog(){
            $("#saveGuru").dialog({
                href:'${pageContext.request.contextPath}/Guru/save.jsp',
                buttons:[{
                    iconCls:'icon-save',
                    text:'保存',
                    handler:function(){
        ...
        //修改
        function openUpdateGuruDialog(id) {
    ...
    <div id="saveGuru" data-options=
            "draggable:false,iconCls:'icon-save',width:600,height:400,title:'保存上师信息'">
    </div>

    <div id="updateGuruDialog" data-options="draggable:false,iconCls:'icon-edit',width:600,height:400,title:'修改上师信息'">
